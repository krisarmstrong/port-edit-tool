                ...
                if ( ipType == UDP_PROTOCOL || ipType == TCP_PROTOCOL )
                {
                    ...
                    //if ( ipType == UDP_PROTOCOL )
                    {
                            ...
                            srcPort == SIP_ALT_PORT || dstPort == SIP_ALT_PORT) )
                        {
                            ...
                            if ( strncmp((char*)(&buf[k+j+FRAME_OFFSET_SIP_STATUS]), szSipOk, strlen(szSipOk) ) == 0 )
                            {
                                printf("-ok");
                                bExtractRtp = true;
                            }
                            if ( strncmp((char*)(&buf[k+j+FRAME_OFFSET_SIP_STATUS]), szSipInBand, strlen(szSipInBand) ) == 0 )
                            {
...
#if 0 // DEBUG
                                //////////
                                char szMsgVia[] = "Via: SIP/2.0/UDP ";
                                char *pMsgVia = strstr((char*)(&buf[k+j+FRAME_OFFSET_SIP_STATUS]), szMsgVia);
                                if ( pMsgVia )
                                {
                                    pMsgVia += strlen(szMsgVia);
                                    char *pColon = strchr(pMsgVia, ':');
                                    if ( pColon )
                                    {
                                        int ipStrLen = pColon - pMsgVia;
                                        char szIpVia[100] = "";
                                        strncpy(szIpVia, pMsgVia, ipStrLen);
                                        printf("DEBUG> ViaIp='%s' ", szIpVia);
                                    }
                                }
                                //////////
#endif
